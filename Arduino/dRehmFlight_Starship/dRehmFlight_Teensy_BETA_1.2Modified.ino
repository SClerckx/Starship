...
float maxRoll = 120.0;     //Max roll angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode 30
...
float Kd_roll_angle = 0.8*0.6;   //Roll D-gain - angle mode (if using controlANGLE2(), set to 0.0)
float B_loop_roll = 0.5;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 0.9*0.5;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 0.3*0.5;   //Pitch I-gain - angle mode
float Kd_pitch_angle = 0.8*0.5*0.6;  //Pitch D-gain - angle mode (if using controlANGLE2(), set to 0.0)
...
float Kp_yaw = 0.3*0.5;           //Yaw P-gain
float Ki_yaw = 0.05*0.5;          //Yaw I-gain
float Kd_yaw = 0.00015*0.5;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)
...
// Code for first iteration of flap control
float Kp_flapPitch = 0.3;
float Kd_flapPitch = 0.2;
float Ki_flapPitch = 0.004;
float pitchTrim = 0.1;
...
// PWM Servo Forward Flaps, left and right
...
// Velocity in global
float velx, vely;
float velx_prev, vely_prev;

// Location vx, vy
float velx_bar, vely_bar;
...
float s1_command_scaled, s2_command_scaled, s3_command_scaled, s4_command_scaled, s5_command_scaled, s6_command_scaled, s7_command_scaled, s8_command_scaled, s9_command_scaled, s10_command_scaled, s11_command_scaled;
int s1_command_PWM, s2_command_PWM, s3_command_PWM, s4_command_PWM, s5_command_PWM, s6_command_PWM, s7_command_PWM, s8_command_PWM, s9_command_PWM, s10_command_PWM,s11_command_PWM;
...
void getIMUdata() {
 ...
 //Accelerometer
  ...
  //AccXW = (-1 + 2 * (q0 * q0 + q1 * q1))*AcX + (2 * (q1 * q2 + q3 * q0))*AcY + (2 * (q1 * q3 - q2 * q0))*AcZ;
  //AccYW =  (2 * (q1 * q2 - q3 * q0))*AcX + (-1 + 2 * (q0 * q0 + q2 * q2))*AcY + (2 * (q2 * q3 + q1 * q0))*AcZ;

  //AccXW = (-1 + 2 * (q0 * q0 + q1 * q1))*AcX + (2 *( q1 * q2 - q3 *q0))*AcY + (2 * (q1 * q3 + q2 * q0))*AcZ;
  //AccYW =  (2 * (q1 * q2 + q3 * q0))*AcX + (-1 + 2 * ( q0 *q0 + q2 * q2))*AcY + (2 * (q2 * q3 - q1 * q0))*AcZ;

  AccXW = q0*q0*AccX + 2*q2*q0*AccZ - 2*q3*q0*AccY + q1*q1*AccX + 2*q2*q1*AccY + 2*q3*q1*AccZ - q3*q3*AccX - q2*q2*AccX;
  AccYW = 2*q1*q2*AccX + q2*q2*AccY + 2*q3*q2*AccZ + 2*q0*q3*AccX - q3*q3*AccY + q0*q0*AccY - 2*q1*q0*AccZ - q1*q1*AccY;
  //AccZW = 2*q1*q3*AccX + 2*q2*q3*AccY + q3*q3*AccZ - 2*q0*q2*AccX - q2*q2*AccZ + 2*q0*q1*AccY - q1*q1*AccZ + q0*q0*AccZ;

  velx = velx_prev + dt * AccXW;
  vely = vely_prev + dt * AccYW;

  velx_bar = velx*cos(yaw_IMU*0.0174533) + vely*sin(yaw_IMU*0.0174533);
  vely_bar = -velx*sin(yaw_IMU*0.0174533) + vely*cos(yaw_IMU*0.0174533);
...
void controlRATE() {
  ...
  integral_roll = constrain(integral_roll, -i_limit, i_limit); //saturate integrator to prevent unsafe buildup
  derivative_roll = (error_roll - error_roll_prev)/dt; 
  roll_PID = .01*(Kp_roll_rate*error_roll + Ki_roll_rate*integral_roll + Kd_roll_rate*derivative_roll); //scaled by .01 to bring within -1 to 1 range + velx_bar*Kd_posx
  ...
  integral_pitch = constrain(integral_pitch, -i_limit, i_limit); //saturate integrator to prevent unsafe buildup
  derivative_pitch = (error_pitch - error_pitch_prev)/dt; 
  pitch_PID = .01*(Kp_pitch_rate*error_pitch + Ki_pitch_rate*integral_pitch + Kd_pitch_rate*derivative_pitch); //scaled by .01 to bring within -1 to 1 range + vely_bar*Kd_posy
...
void printWorldAccelData() {
    if (current_time - print_counter > 10000) {
    print_counter = micros();
    Serial.print(F("AccXW: "));
    Serial.print(AccXW);
    Serial.print(F(" AccYW: "));
    Serial.println(AccYW);
  }
}
...
void printVelBarData() {
    if (current_time - print_counter > 10000) {
    print_counter = micros();
    Serial.print(F(" velx_bar: "));
    Serial.print(velx_bar);
    Serial.print(F(" vely_bar: "));
    Serial.println(vely_bar);
  }
}
