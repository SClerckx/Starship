...
void setup() {
  ...
  Serial.println("Exited IMU init");
...
void IMUinit() {
  ...
  Serial.println("BNOO8x Successful Init");
  //delay(100);
}
...
void controlMixer() {
  ...
  s6_command_scaled = 0;
  s7_command_scaled = 0;
  s8_command_scaled = pitch_PID; //+ roll_PID;
  s9_command_scaled = -pitch_PID; //- roll_PID;
  s10_command_scaled = pitch_PID; //+ roll_PID ;
  s11_command_scaled = pitch_PID; //- roll_PID ;*/
...
void commandMotors() {
  ...
  while (wentLow < 4 ) { //keep going until final (6th) pulse is finished, then done REVERT TO 6
    timer = micros();
...
void printRollPitchYaw() {
    if (current_time - print_counter > 100) {
    print_counter = micros();
    Serial.print(F("roll:"));
