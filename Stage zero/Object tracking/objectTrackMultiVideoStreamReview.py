...
def animate():
	...
	ax.scatter(threaded_camera1.position[0], threaded_camera1.position[1], threaded_camera1.position[2], label = "threaded_camera1")
	ax.scatter(threaded_camera2.position[0], threaded_camera2.position[1], threaded_camera2.position[2], label = "threaded_camera2")

	t1 = 4
	ax.plot([threaded_camera1.position[0], threaded_camera1.getLine(t1)[0]], [threaded_camera1.position[1], threaded_camera1.getLine(t1)[1]], [threaded_camera1.position[2], threaded_camera1.getLine(t1)[2]], label = "threaded_camera1Line")
	ax.plot([threaded_camera2.position[0], threaded_camera2.getLine(t1)[0]], [threaded_camera2.position[1], threaded_camera2.getLine(t1)[1]], [threaded_camera2.position[2], threaded_camera2.getLine(t1)[2]], label = "threaded_camera2Line")

	plt.legend(loc='upper left')
	plt.pause(0.2)

def animate2(positions):
	plt.cla()

	ax = plt.axes(projection='3d')
	ax.scatter([position[0] for position in positions], 
	[position[1] for position in positions], 
	[position[2] for position in positions], 
	label = "threaded_camera1")

	plt.legend(loc='upper left')
	plt.pause(0.2)

def plotSolutions(positions):
	ax = plt.axes(projection='3d')
	ax.scatter([position[0] for position in positions], 
	[position[1] for position in positions], 
	[position[2] for position in positions], )
...
if __name__ == '__main__':
	threaded_camera1 = ThreadedCamera(url1, position=np.array([2.19, 0, 0]), e1 = np.array([-1, 0, 0]), e2 = np.array([0, 1, 0]))
	threaded_camera2 = ThreadedCamera(url2, position=np.array([0, 1.56, 0]), e1 = np.array([0, -1, 0]), e2 = np.array([-1, 0, 0]))
	prevTime = time.time()
	startTime = prevTime
